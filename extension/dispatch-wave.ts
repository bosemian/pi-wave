...
export default function (pi: ExtensionAPI) {
  pi.registerTool({
    ...
    description:
      ...
      '{ name, cwd: "/abs/project/dir" (omitted or "." = the directory this session runs in - agents work there), on_failure: "stop"|"continue", display: "auto"(herdr panes inside Herdr, headless otherwise)|"herdr"|"headless", orchestrator: true|{model,thinking,synthesis_model,synthesis_thinking} (spawns a wider orchestrator pane, default openai-codex/gpt-5.5 at high, which reviews agents that have no review_cmd against their done_when, keeps agents in sync - after each wave it cross-checks their results (and any issues a QA agent reports) and sends fixes back to the agents that own the work, even in earlier waves - and writes the final synthesis; set synthesis_model/synthesis_thinking to hand ONLY the synthesis to a dedicated synthesizer pane), waves: [[assignment, ...], ...] }. ' +
